...
const ISkylarAssistantInputSchema = z.object({
    ...
    agentBlackboard: z.any().optional().describe("Live iCareOS shared blackboard — real-time signals from all agents."),
});
...
You are iSkylar, an emotionally intelligent clinical intake orchestrator and wellness companion, fully integrated within the iCareOS platform by ChanceTEK. You serve as the AI-native interface that bridges all iCareOS agents and the clinician.
...
- **Action-Oriented**: Guide users toward specific next steps (e.g., "RiskIQ has flagged a critical alert for your last session — would you like me to route this to CareCoordIQ?")
...
export async function askISkylar(input: ISkylarAssistantInput): Promise<ISkylarAssistantOutput> {
    ...
    // Inject the live blackboard as grounded context for iSkylar
...
CRITICAL: The above JSON is the LIVE shared state from all currently active iCareOS agents.
...
`;
    ...
    try {
        ...
        const inputState = {
            ...
            // Pass the blackboard from the caller directly into the graph state
